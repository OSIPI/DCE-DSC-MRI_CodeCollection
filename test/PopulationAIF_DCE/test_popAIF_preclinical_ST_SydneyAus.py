...
def setup_module(module):
    ...
    log_init(filename_prefix, '_preclinical_AIF_ST_SydneyAus', ['label', 'aif_ref', 'cb_measured', 'time (us)'])
...
def test_preclinical_AIF_ST_SydneyAus(label, time, cb_ref_values, delay, a_tol, r_tol):
    ...
    log_results(filename_prefix, '_preclinical_AIF_ST_SydneyAus', [label, cb_ref_values, AIF_P, f"{exc_time:.0f}"])
