...
test_data = (
    t1_data.t1_brain_data() + t1_data.t1_quiba_data() + t1_data.t1_prostate_data()
)
...
def setup_module(module):
    ...
    log_init(
        filename_prefix,
        "_MJT_Edinburgh_UK_t1_VFA_nonlin",
        ["label", "time (us)", "r1_ref", "r1_measured"],
    )
    log_init(
        filename_prefix,
        "_MJT_Edinburgh_UK_t1_VFA_lin",
        ["label", "time (us)", "r1_ref", "r1_measured"],
    )
    log_init(
        filename_prefix,
        "_MJT_Edinburgh_UK_t1_VFA_2fa",
        ["label", "time (us)", "r1_ref", "r1_measured"],
    )
...
def test_MJT_Edinburgh_UK_t1_VFA_nonlin(
    label, fa_array, tr_array, s_array, r1_ref, s0_ref, a_tol, r_tol
):
    # NOTES:
    ...
    r1_nonlin_meas = 1.0 / t1_nonlin_meas
    log_results(
        filename_prefix,
        "_MJT_Edinburgh_UK_t1_VFA_nonlin",
        [[label, f"{exc_time:.0f}", r1_ref, r1_nonlin_meas]],
    )  # log results
...
def test_MJT_Edinburgh_UK_t1_VFA_lin(
    label, fa_array, tr_array, s_array, r1_ref, s0_ref, a_tol, r_tol
):
    # NOTES:
...
# In the following test, we specify 1 case that is expected to fail...
@osipi_parametrize(arg_names, test_data, xf_labels=["Pat5_voxel5_prostaat"])
def test_MJT_Edinburgh_UK_t1_VFA_2fa(
    label, fa_array, tr_array, s_array, r1_ref, s0_ref, a_tol, r_tol
):
    # NOTES:
    #   Expected fails: 1 low-SNR prostate voxel
    ...
    # run test (non-linear)
    tic = perf_counter()
    [s0_2fa_meas, t1_2fa_meas] = VFA2Points(
        np.array([fa_array[0], fa_array[-1]]), tr
    ).proc(np.array([s_array[0], s_array[-1]]))
    exc_time = 1e6 * (perf_counter() - tic)
    r1_2fa_meas = 1.0 / t1_2fa_meas
    log_results(
        filename_prefix,
        "_MJT_Edinburgh_UK_t1_VFA_2fa",
        [[label, f"{exc_time:.0f}", r1_ref, r1_2fa_meas]],
    )  #
    # log results
