...
def setup_module(module):
    ...
    log_init(
        filename_prefix,
        "_ST_USyd_AUS_t1_VFA_nonlin",
        ["label", "time (" "us)", "r1_ref", "r1_measured"],
    )
    log_init(
        filename_prefix,
        "_ST_USyd_AUS_t1_VFA_lin",
        ["label", "time (" "us)", "r1_ref", "r1_measured"],
    )
...
def test_ST_Sydney_AUS_t1_VFA_nonlin(
    label, fa_array, tr_array, s_array, r1_ref, s0_ref, a_tol, r_tol
):
    # NOTES:

    # prepare input data
    tr = tr_array[0] * 1000.0  # convert s to ms

    # run test (non-linear)
    tic = perf_counter()
    [s0_nonlin_meas, t1_nonlin_meas] = VFAT1mapping(
        fa_array, s_array, tr, method="nonlinear"
    )
    exc_time = 1e6 * (perf_counter() - tic)
    r1_nonlin_meas = 1000.0 / t1_nonlin_meas  # convert T1 (ms) to R1 (/s)
    log_results(
        filename_prefix,
        "_ST_USyd_AUS_t1_VFA_nonlin",
...
def test_ST_Sydney_AUS_t1_VFA_lin(
    label, fa_array, tr_array, s_array, r1_ref, s0_ref, a_tol, r_tol
):
    # NOTES:
    ...
    exc_time = 1e6 * (perf_counter() - tic)
    r1_lin_meas = 1000.0 / t1_lin_meas  # convert T1 (ms) to R1 (/s)
    log_results(
        filename_prefix,
        "_ST_USyd_AUS_t1_VFA_lin",
