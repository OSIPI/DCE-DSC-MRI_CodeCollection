...
from . import t1_data
from osipi.original.OG_MO_AUMC_ICR_RMH_NL_UK.ExtendedTofts.DCE import R1_two_fas, R1_VFA
...
def setup_module(module):
    ...
    log_init(
        filename_prefix,
        "_OG_MO_AUMC_ICR_RHM_NL_UK_t1_VFA_2fa",
        ["label", "time (us)", "r1_ref", "r1_measured"],
    )
    log_init(
        filename_prefix,
        "_OG_MO_AUMC_ICR_RHM_NL_UK_t1_VFA_nonlin",
        ["label", "time (us)", "r1_ref", "r1_measured"],
    )
...
def test_OG_MO_AUMC_ICR_RMH_NL_UK_t1_VFA_2fa(
    ...
    fa_array_rad = fa_array[[0, -1]] * np.pi / 180.0  # use first and last FA only
...
def test_OG_MO_AUMC_ICR_RMH_NL_UK_t1_VFA(
    label, fa_array, tr_array, s_array, r1_ref, s0_ref, a_tol, r_tol
):
    # NOTES:
    #   Code requires signal array with min 2 dimensions (including FA)
    ...
    # prepare input data
    tr = tr_array[0]
    fa_array_rad = fa_array * np.pi / 180.0
    ...
    log_results(
        filename_prefix,
        "_OG_MO_AUMC_ICR_RHM_NL_UK_t1_VFA_nonlin",
        [[label, f"{exc_time:.0f}", r1_ref, r1_meas]],
    )  # log results to csv
