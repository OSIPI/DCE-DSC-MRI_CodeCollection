...
from osipi.original.LEK_UoEdinburgh_UK.SignalToConcentration import SI2Conc
...
def setup_module(module):
    ...
    log_init(
        filename_prefix,
        "_LEK_UoEdinburgh_UK",
        ["label", "time (us)", "conc_ref", "conc_meas"],
    )
...
def test_LEK_UoEdinburgh_UK_SI2Conc(
    label, fa, tr, T1base, BLpts, r1, s_array, conc_array, a_tol, r_tol
):
    ...
    conc_array = (
        conc_array * r1
    )  # This function doesn't include r1, so multiply it out before testing
    np.testing.assert_allclose(
        [conc_curve[1:]], [conc_array[1:]], rtol=r_tol, atol=a_tol
    )
