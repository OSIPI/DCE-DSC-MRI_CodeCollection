...
def test_MJT_UoEdinburgh_UK_extended_tofts_kety_model(
    label,
    t_array,
    C_array,
    ca_array,
...
def test_MJT_UoEdinburgh_UK_extended_tofts_kety_model_delay(
    label,
    t_array,
    C_array,
    ca_array,
    ...
    t_array = t_array  # in seconds
    aif = aifs.PatientSpecific(t_array, ca_array)
    pk_model = pk_models.ExtendedTofts(
        t_array, aif, upsample_factor=3, fixed_delay=None
    )
    ...
    # run test
    vp_meas, ps_meas, ve_meas, delay_meas, C_t_fit = dce_fit.ConcToPKP(pk_model).proc(
        C_array
    )
    Ktrans_meas = ps_meas
    exc_time = 1e6 * (perf_counter() - tic)  # measure execution time
    ...
    np.testing.assert_allclose(
        [delay_meas], [arterial_delay_ref], rtol=r_tol_delay, atol=a_tol_delay
    )
