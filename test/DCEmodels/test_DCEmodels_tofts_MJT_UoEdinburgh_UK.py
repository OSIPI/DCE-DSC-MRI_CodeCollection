...
def test_MJT_UoEdinburgh_UK_tofts_model(
    label,
    t_array,
    C_array,
    ca_array,
...
def test_MJT_UoEdinburgh_UK_tofts_model_delay(
    label,
    t_array,
    C_array,
    ca_array,
    ...
    # run code
    tic = perf_counter()
    Ktrans_meas, ve_meas, delay_meas, C_t_fit = dce_fit.ConcToPKP(pk_model).proc(
        C_array
    )
    exc_time = 1e6 * (perf_counter() - tic)  # measure execution time
