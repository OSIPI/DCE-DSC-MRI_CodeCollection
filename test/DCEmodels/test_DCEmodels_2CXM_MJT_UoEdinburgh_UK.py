...
def test_MJT_UoEdinburgh_UK_2cxm_model(
    label,
    t_array,
    C_t_array,
    cp_aif_array,
    ...
    # run code
    tic = perf_counter()
    vp_meas, ps_meas, ve_meas, fp_meas, C_t_fit = dce_fit.ConcToPKP(pk_model).proc(
        C_t_array
    )
...
def test_MJT_UoEdinburgh_UK_2cxm_model_delay(
    label,
    t_array,
    C_t_array,
    cp_aif_array,
    ...
    # run code
    tic = perf_counter()
    vp_meas, ps_meas, ve_meas, fp_meas, delay_meas, C_t_fit = dce_fit.ConcToPKP(
        pk_model
    ).proc(C_t_array)
    exc_time = 1e6 * (perf_counter() - tic)  # measure execution time
