...
class Aif:
    def __init__(
        ...
        if aif_type == AifType.POP:
            if self.base_aif_.size:
                warnings.warn(
                    "base_aif set but will be overwritten because aif_type=POP. Use aif_type=ARRAY if setting base_aif"
                )

            if self.num_times() and (self.num_times() < self.prebolus_):
                raise ValueError(
                    f"Size of times {self.num_times()} must be at least prebolus {self.prebolus_}"
                )
            ...
            if self.base_aif_.size:
                warnings.warn(
                    "base_aif set but will be overwritten because aif_type=FILE and filename is set. Use aif_type=ARRAY if setting base_aif"
                )
    ...
    def compute_population_AIF(self, offset=0):
        ...
        t_offset = self.times_.reshape(1, self.num_times()) - offset - prebolus_time
    ...
    def write_AIF(self, filename):
        ...
        combined_AIF = np.concatenate(
            (np.reshape(self.times_, (-1, 1)), np.reshape(self.base_aif_, (-1, 1))),
            axis=1,
        )
    ...
    def resample_AIF(self, offset):
        ...
        if self.type_ == AifType.POP:
            ...
            t_offset = (
                self.times_.reshape(1, self.num_times()) - offset
            )  # output is nv x nt
    ...
    def resample_PIF(self, offset, offsetAIF, resampleIRF):
        ...
        if (
            resampleIRF
            or (len(self.PIF_IRF_) != n_t)
            or np.any(np.isnan(self.PIF_IRF_))
        ):
            ...
            for i_t in range(n_t):
                ...
                if t < 0.08:
                    self.PIF_IRF_[
                        :, i_t
                    ] = 0  # This might have been set to NaN, so make sure we set back to zero
                elif t < 0.17:
                    self.PIF_IRF_[:, i_t] = 24.16 * t - 2.01
                else:
                    self.PIF_IRF_[:, i_t] = 2.83 * np.exp(-10.80 * t) + 2.12 * np.exp(
                        -1.82 * t
                    )
            ...
            self.PIF_IRF_ /= (
                irf_sum  # Broadcast assignment division? Numpy I love you... ;o)
            )
        ...
        for i_t in range(n_t):
            ...
            for j_t in range(i_t + 1):
                pif_sum += (
                    self.resampled_AIF_[:, j_t] * self.PIF_IRF_[:, k_t]
                )  # (nv x 1)*(nv x 1)
                k_t -= 1
