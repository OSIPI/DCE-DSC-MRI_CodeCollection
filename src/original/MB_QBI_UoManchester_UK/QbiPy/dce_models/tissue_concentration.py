...
def signal_to_concentration(
    ...
    if use_M0_ratio > 0:
        ...
        if M0.size != num_voxels:
            # Flag error - throw exception?, return empty signals
            raise ValueError(
                f"Size of M0 ({M0.size}) does not match number of columns in S_t ({num_voxels})"
            )
...
def compute_IAUC(
    ...
    iauc_idx = np.nonzero((dyn_times - dyn_times[aif_injection_image - 1]) > interval)[
        0
    ][0]
