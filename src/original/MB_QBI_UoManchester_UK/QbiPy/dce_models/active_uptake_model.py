...
#
# -------------------------------------------------------------------------------
def params_from_DIBEM(F_pos, F_neg, K_pos, K_neg, using_Fp=False, warn_mode="warn"):
    """
    ...
    if np.any(swap_idx):
        if warn_mode == "warn":
            ...
            raise ValueError(
                "Warn mode {warn_mode} not recognised. Must be "
                "warn"
                ", "
                "quiet"
                " or "
...
def concentration_from_model(
    ...
    F_pos, F_neg, K_pos, K_neg = params_to_DIBEM(Fp, PS, Ve, Vp)
...
#
# ---------------------------------------------------------------------------
def construct_LLS_matrix(
    Ctis_t: np.array, aif: dce_aif.Aif, f_a: float, tau_a: float, tau_v: float
):
    """
